import type { ActivityPage, ApiError, ChartPage, CreatorProfile, CTOCheckpointPage, CTOFeePullPage, CTOProposal, CTOProposalPage, CTOStatus, CTOTreasury, CTOTreasuryTransferPage, Pricing, Token, TokenPage, TradePage, WalletHoldingPage } from "@cooket/types";
import { z } from "zod";
...
const walletHoldingPageSchema = z.object({ indexed_through_block: z.number().int().nonnegative(), items: z.array(z.object({ token_address: addressSchema, name: z.string(), symbol: z.string(), image_url: z.string().optional(), raw_balance: integerStringSchema, token_decimals: z.number().int().nonnegative(), lifecycle: z.string(), current_price: integerStringSchema.nullable(), estimated_value: integerStringSchema.nullable(), metrics: z.object({ trade_count: z.number().int().nonnegative(), volume: integerStringSchema, holder_count: z.number().int().nonnegative().nullable() }) })), next_cursor: z.string().optional() });
...
export const api = {
  ...
  walletHoldings: (address: string, query = "") => request<WalletHoldingPage>(`/api/v1/wallets/${encodeURIComponent(address)}/holdings${query}`, walletHoldingPageSchema),
  ...
  finalizeMetadata: (draft:string,tokenAddress:string,transactionHash:string,signature:string) => mutation(`/api/v1/token-metadata/${encodeURIComponent(draft)}/finalize`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token_address:tokenAddress,transaction_hash:transactionHash,signature})},tokenSchema),
...
export type { ActivityPage, ApiError, ChartPage, CreatorProfile, CTOCheckpointPage, CTOFeePullPage, CTOProposal, CTOProposalPage, CTOStatus, CTOTreasury, CTOTreasuryTransferPage, Pricing, Token, TokenPage, TradePage, WalletHoldingPage };
