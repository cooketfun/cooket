...
export function GraduatedTokenSwap({ tokenAddress, canonicalPoolAddress, symbol }: { tokenAddress: Address; canonicalPoolAddress?: Address; symbol: string }) {
  ...
  const requestQuote = useCallback(async () => {
    ...
    try {
      ...
      if (side === "buy" && stateQuery.data && parsed > stateQuery.data.eth) throw new Error("Insufficient native USDC balance.");
  ...
  return <section className="terminal-panel p-5" aria-label="Graduated token swap">
    ...
    {guard ? <p className="status-box status-warning mt-4 text-sm">{guard}</p> : <>
      <label className="mt-5 block text-xs text-zinc-500">Pay <span className="float-right">{side === "buy" ? "native USDC" : symbol}</span><input className="mt-2 w-full rounded-lg border border-white/10 bg-black/20 p-3 text-white" aria-label={side === "buy" ? "Native USDC amount" : `${symbol} amount`} inputMode="decimal" value={amount} onChange={(event) => changeAmount(event.target.value)} placeholder="0.0" /></label>
      ...
      <details className="mt-4 text-xs text-zinc-600"><summary>Execution details</summary><p className="mt-2 break-all">Pool {canonicalPoolAddress}<br />SwapRouter02 {poolQuery.data?.router}<br />{selectedCooketChainName} · quote deadline 5 minutes</p></details>
    </>}
    <TransactionModal open={modal.open} title={`${side === "sell" && (status === "awaiting_approval" || status === "approval_confirming") ? "Approve" : "Swap"} ${symbol}`} phase={modal.phase} wallet={wallet} hash={hash} error={error} onClose={() => dispatchModal({ type: "close" })} onConfirm={() => void submit()} confirmLabel={side === "sell" && quote && stateQuery.data && stateQuery.data.allowance < quote.amountIn ? "Start approval + swap" : "Confirm swap"} confirmDisabled={Boolean(guard)} details={quote ? [
      { label: "Input", value: `${formatUnits(quote.amountIn, side === "buy" ? 18 : stateQuery.data?.decimals ?? 18)} ${side === "buy" ? "native USDC" : symbol}` },
      { label: "Expected output", value: `${formatUnits(quote.amountOut, side === "buy" ? stateQuery.data?.decimals ?? 18 : 18)} ${side === "buy" ? symbol : "native USDC"}` },
