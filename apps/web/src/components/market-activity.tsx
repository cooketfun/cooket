...
import { formatExactTokenAmount, formatMarketTradeUSDC, formatTokenAmount, formatTokenSymbol, formatTradeUsdc } from "@/lib/format";
...
export function MarketActivity() {
  ...
  return <div className="panel overflow-hidden p-0"><div className="safe-scroll"><table className="w-full min-w-[42rem] text-left text-sm"><thead className="border-b border-white/8 bg-white/[0.02] text-[0.68rem] uppercase tracking-[0.12em] text-zinc-600"><tr><th className="px-4 py-3 font-medium">Token</th><th className="px-4 py-3 font-medium">Side</th><th className="px-4 py-3 text-right font-medium">Token amount</th><th className="px-4 py-3 text-right font-medium">Trade value</th><th className="px-4 py-3 text-right font-medium">Block</th></tr></thead><tbody className="divide-y divide-white/6">{query.data.map((trade) => <tr className="transition-colors hover:bg-white/[0.025]" key={`${trade.transaction_hash}:${trade.log_index}`}><td className="px-4 py-3"><Link className="font-medium text-zinc-100 hover:text-cyan-200" href={`/token/${trade.token.address}`}>{trade.token.name} <span className="text-xs text-zinc-500">{formatTokenSymbol(trade.token.symbol)}</span></Link></td><td className="px-4 py-3"><span className={trade.side === "buy" ? "text-emerald-300" : "text-rose-300"}>{trade.side.toUpperCase()}</span></td><td className="px-4 py-3 text-right text-zinc-300" title={formatExactTokenAmount(trade.token_amount, 18, trade.token.symbol)}>{formatTokenAmount(trade.token_amount, 18, trade.token.symbol)}</td><td className="px-4 py-3 text-right"><span className="font-medium text-zinc-100" title={formatTradeUsdc(trade.reserve_amount, trade.source)}>{formatMarketTradeUSDC(trade.reserve_amount, trade.source)}</span></td><td className="px-4 py-3 text-right font-mono text-xs text-zinc-500"><a href={explorerTransactionURL(trade.transaction_hash)} target="_blank" rel="noreferrer" className="hover:text-cyan-300">{trade.block_number} ↗</a></td></tr>)}</tbody></table></div></div>;
}
