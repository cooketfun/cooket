...
The launch form creates a metadata draft first, then asks your browser wallet to call `CooketFactoryV3.createToken` on Arc Testnet. Launch execution remains fail-closed until later phases.
...
| Dev buy | Optional native USDC amount for a separate curve buy after launch |
...
sequenceDiagram
  ...
  participant F as CooketFactoryV3
...
The factory atomically deploys and registers the ERC-20 token and its dedicated curve, places the full supply in the curve, registers fee and graduation relationships, and reserves the canonical token/native-USDC pool.
