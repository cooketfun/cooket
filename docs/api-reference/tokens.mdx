...
When present, `graduation.native_usdc_amount` is the 18-decimal Arc native-USDC principal as a decimal integer string. It is not ETH, not WETH, and not 6-decimal ERC-20 USDC. The API does not emit `eth_amount`.

Read-only CTO status for the same token is a separate resource: [CTO](/docs/api-reference/cto).
