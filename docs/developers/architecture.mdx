...
description: "Browser, Arc Testnet, indexer, storage, API, and cache boundaries."
---
...
flowchart TB
  subgraph Client
    B[Browser]
    W[Next.js web]
    BW[Active external EVM wallet]
    B --> W
    W --> BW
  end
  BW -. reads only .-> RPC[Arc Testnet RPC]
  RPC -. no configured Cooket contracts .-> IX[Go indexer in idle mode]
  IX --> PG[(PostgreSQL)]
  PG --> API[Go API]
  API --> W
