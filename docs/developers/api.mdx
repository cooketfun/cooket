...
The Go API uses chi and PostgreSQL. It serves health/readiness, immutable stored objects, metadata draft/finalization writes, and indexed token resources scoped to Arc Testnet. It rejects Base chain IDs. The inherited ETH/USD endpoint remains schema-compatible but returns unavailable because no Arc oracle is configured in Phase 0.
