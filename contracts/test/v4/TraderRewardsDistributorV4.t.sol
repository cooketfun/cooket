...
import {ITraderRewardsDistributorV4} from "../../src/v4/interfaces/ITraderRewardsDistributorV4.sol";
import {ITraderRewardsVaultV4} from "../../src/v4/interfaces/ITraderRewardsVaultV4.sol";
...
contract TraderRewardsDistributorV4Test is CooketV4TestBase {
    ...
    function testVaultAndDistributorBindingsAndFundingSourcesAreFixed() public {
        ...
        assertEq(rewardsVault.protocolVersionHash(), keccak256("endpoint-cp-v4"));
        assertEq(rewardsVault.feePolicyHash(), keccak256("cooket-fee-design-b-v3"));
        assertEq(rewardsDistributor.protocolVersionHash(), keccak256("endpoint-cp-v4"));
        assertEq(rewardsDistributor.feePolicyHash(), keccak256("cooket-fee-design-b-v3"));

        vm.prank(buyer);
        vm.expectRevert(ITraderRewardsVaultV4.UnauthorizedFundingSource.selector);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ITraderRewardsVaultV4.InsufficientBacking.selector, address(canonicalUsdc), uint256(0), uint256(1)
            )
        );
        rewardsVault.recordERC20Funding(address(token), address(canonicalUsdc), 1);
    ...
    function testValidMerkleClaimsPayAuthenticatedClaimantsAndRejectReplay() public {
        ...
        vm.prank(buyer);
        vm.expectRevert(ITraderRewardsDistributorV4.AlreadyClaimed.selector);
        rewardsDistributor.claim(1, address(token), address(0), amountOne, proofOne);
    }
    ...
    function testInvalidProofAndWrongDistributionDimensionsRevert() public {
        ...
        vm.prank(buyer);
        vm.expectRevert(ITraderRewardsDistributorV4.InvalidMerkleProof.selector);
        rewardsDistributor.claim(7, address(token), address(0), 1 ether - 1, emptyProof);
        vm.prank(buyer);
        vm.expectRevert(ITraderRewardsDistributorV4.DistributionNotPublished.selector);
    ...
    function testRootPublicationIsGovernedAndImmutable() public {
        ...
        vm.expectRevert(ITraderRewardsDistributorV4.InvalidRoot.selector);
        rewardsDistributor.publishRoot(1, address(token), address(0), bytes32(0));
        rewardsDistributor.publishRoot(1, address(token), address(0), root);
        vm.expectRevert(ITraderRewardsDistributorV4.DistributionAlreadyPublished.selector);
    ...
    function testInsufficientFundingRevertsClaimWithoutBurningAuthorization() public {
        ...
        vm.prank(buyer);
        vm.expectRevert(ITraderRewardsVaultV4.InsufficientRewardsBalance.selector);
    ...
    function testMaliciousReceiverCannotReenterOrKeepAuthorizationOnFailedPayout() public {
        ...
        ReentrantRewardClaimantV4 attacker = new ReentrantRewardClaimantV4(rewardsDistributor);
        ...
        vm.expectRevert(ITraderRewardsVaultV4.NativeTransferFailed.selector);
    ...
    function testERC20RewardClaimAndOnlyDistributorPayout() public {
        ...
        vm.prank(buyer);
        vm.expectRevert(ITraderRewardsVaultV4.UnauthorizedDistributor.selector);
...
contract ReentrantRewardClaimantV4 {
    TraderRewardsDistributorV4 public immutable distributor;
    bool public rejectPayment;
    bool public reentrySucceeded;

    constructor(TraderRewardsDistributorV4 distributor_) {
    ...
    receive() external payable {
        ...
        (reentrySucceeded,) = address(distributor)
            .call(abi.encodeCall(TraderRewardsDistributorV4.claim, (4, address(0), address(0), uint256(1), proof)));
    }
}
