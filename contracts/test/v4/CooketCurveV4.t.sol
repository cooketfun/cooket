...
contract ForceNativeUsdcCurveV4 {
    constructor() payable {}
...
contract CooketCurveV4Test is CooketV4TestBase {
    function testForcedNativeUsdcDoesNotChangeQuotesOrGraduationAccounting() public {
        ICooketCurveV4.BuyQuote memory beforeQuote = curve.quoteBuy(GRADUATION_GROSS);
        ForceNativeUsdcCurveV4 force = new ForceNativeUsdcCurveV4{value: 2 * NATIVE_USDC_UNIT}();
        force.force(payable(address(curve)));
        ICooketCurveV4.BuyQuote memory afterQuote = curve.quoteBuy(GRADUATION_GROSS);
    ...
    function testCanonicalConstantsAndInitialEndpoint() public view {
        assertEq(curve.PROTOCOL_VERSION(), "endpoint-cp-v4");
        assertEq(curve.protocolVersionHash(), keccak256("endpoint-cp-v4"));
        ...
        assertEq(curve.VIRTUAL_NATIVE_USDC_RESERVE(), 4_830 * NATIVE_USDC_UNIT);
        ...
        assertEq(curve.INITIAL_NATIVE_USDC_PRICE(), 4_528_125_000_000);
        assertEq(curve.TERMINAL_NATIVE_USDC_PRICE(), 72_450_000_000_000);
        assertEq(curve.spotPrice(), 4_528_125_000_000);
        assertEq(curve.spotPrice() * 1_000_000_000, 4_528_125_000_000_000_000_000);
    }

    function testZeroCreatorRecipientIsExplicitlyRejected() public {
        vm.expectRevert(ICooketCurveV4.InvalidRecipient.selector);
        new CooketCurveV4(address(factory), address(token), address(0), address(feeManager), address(graduationManager));
    }
    ...
    function testGrossRequiredForNetRejectsUnrepresentableGrossWithoutOverflowPanic() public {
        vm.expectRevert(ICooketCurveV4.InvalidAmount.selector);
        curve.grossRequiredForNet(type(uint256).max);
    }

    function testAcceptedSimulatorBuyVector() public {
        ICooketCurveV4.BuyQuote memory quote = curve.quoteBuy(0.1 ether);
        ...
        assertEq(quote.tokensOut, 21_862_905_915_593_034_428_486);

        vm.prank(buyer);
        ICooketCurveV4.BuyQuote memory executed = curve.buy{value: 0.1 ether}(quote.tokensOut, block.timestamp);
        ...
        assertEq(curve.spotPrice(), 4_528_310_626_903);
    }
    ...
    function testDifferentialAcceptedLaunchVectors() public view {
        ...
        uint256[9] memory expectedTokens = [
            uint256(2_186_335_399_245_399_492),
            21_863_353_589_136_230_781,
            218_633_495_559_594_077_847,
            2_186_330_922_427_301_857_083,
            10_931_564_987_066_902_730_499,
            21_862_905_915_593_034_428_486,
            54_655_584_418_810_837_332_869,
            109_305_568_062_900_385_985_287,
            218_588_736_470_164_500_444_008
        ];
        for (uint256 i; i < grossInputs.length; ++i) {
            ICooketCurveV4.BuyQuote memory quote = curve.quoteBuy(grossInputs[i]);
    ...
    function testOneWeiBelowExactAndOneWeiAboveGraduationBoundary() public {
        ICooketCurveV4.BuyQuote memory belowQuote = curve.quoteBuy(GRADUATION_GROSS - 1);
        ...
        ICooketCurveV4.BuyQuote memory lastWei = curve.quoteBuy(1);
        ...
        assertEq(terminalFdv, 72_450 * NATIVE_USDC_UNIT);
        assertEq(terminalFdv / GRADUATION_NATIVE_USDC_RESERVE, 5);
    }

    function testExactBoundaryGraduatesAndForwardsPrincipal() public {
        vm.prank(buyer);
        ICooketCurveV4.BuyQuote memory quote = curve.buy{value: GRADUATION_GROSS}(0, block.timestamp);
    ...
    function testOneWeiAboveBoundaryRefundsAndFeesOnlyAcceptedGross() public {
        ...
        vm.prank(buyer);
        ICooketCurveV4.BuyQuote memory quote = curve.buy{value: GRADUATION_GROSS + 1}(0, block.timestamp);
    ...
    function testTinyFeeWeiAlwaysBelongsToProtocolRemainderSink() public {
        ICooketCurveV4.FeeSplit memory split = curve.splitFee(100);
    ...
    function testFeeDesignBExactAllocationAndProtocolRemainder() public view {
        ICooketCurveV4.FeeSplit memory exactSplit = curve.splitFee(10_000);
        ...
        ICooketCurveV4.FeeSplit memory roundedSplit = curve.splitFee(10_100);
    ...
    function testBuyAndSellQuoteExecutionParityAndRoundTripAccounting() public {
        ICooketCurveV4.BuyQuote memory buyQuote = curve.quoteBuy(1 ether);
        vm.prank(buyer);
        ICooketCurveV4.BuyQuote memory buyExecution = curve.buy{value: 1 ether}(buyQuote.tokensOut, block.timestamp);
        assertEq(keccak256(abi.encode(buyQuote)), keccak256(abi.encode(buyExecution)));

        vm.prank(buyer);
        token.approve(address(curve), buyQuote.tokensOut);
        ICooketCurveV4.SellQuote memory sellQuote = curve.quoteSell(buyQuote.tokensOut);
        vm.prank(buyer);
        ICooketCurveV4.SellQuote memory sellExecution =
    ...
    function testTradingAfterGraduationAndSecondGraduationPathReject() public {
        vm.prank(buyer);
        curve.buy{value: GRADUATION_GROSS}(0, block.timestamp);
        vm.expectRevert(ICooketCurveV4.TradingClosed.selector);
        curve.quoteBuy(1);
        vm.expectRevert(ICooketCurveV4.TradingClosed.selector);
        curve.quoteSell(1);
        assertEq(graduationManager.calls(), 1);
    ...
    function testCompetingFinalGraduationBuysPreserveExactReserve() public {
        ...
        vm.deal(rival, 20_000 * NATIVE_USDC_UNIT);
        ICooketCurveV4.BuyQuote memory firstQuote = curve.quoteBuy(GRADUATION_GROSS);
        ICooketCurveV4.BuyQuote memory staleSecondQuote = firstQuote;
    ...
    function testStrictMinOutRevertsAfterAdversarialStateMovement() public {
        ICooketCurveV4.BuyQuote memory victimQuote = curve.quoteBuy(0.1 ether);
        address attacker = makeAddr("mevAttacker");
        vm.deal(attacker, 10 ether);
        _buy(attacker, curve, 0.5 ether);

        vm.prank(buyer);
        vm.expectRevert(ICooketCurveV4.SlippageExceeded.selector);
    ...
    function testLooseMinOutMayExecuteAtWorsePrice() public {
        ICooketCurveV4.BuyQuote memory original = curve.quoteBuy(0.1 ether);
        address attacker = makeAddr("looseSlippageAttacker");
        vm.deal(attacker, 10 ether);
        _buy(attacker, curve, 0.5 ether);

        ICooketCurveV4.BuyQuote memory moved = curve.quoteBuy(0.1 ether);
        assertLt(moved.tokensOut, original.tokensOut);

        vm.prank(buyer);
        ICooketCurveV4.BuyQuote memory executed = curve.buy{value: 0.1 ether}(0, block.timestamp);
...
contract CooketTokenV4Test is CooketV4TestBase {
    function testFixedSupplySingleMintAndZeroCreatorAllocation() public view {
        assertEq(token.protocolVersionHash(), keccak256("endpoint-cp-v4"));
