...
import {CurveDeployerV4} from "../../src/v4/CurveDeployerV4.sol";
...
import {ICurveDeployerV4} from "../../src/v4/interfaces/ICurveDeployerV4.sol";
import {IGraduationManagerV4} from "../../src/v4/interfaces/IGraduationManagerV4.sol";
import {ITokenDeployerV4} from "../../src/v4/interfaces/ITokenDeployerV4.sol";
...
import {MockConfiguredUniswapV3PoolV4, MockUniswapV3PoolV4} from "./mocks/MockUniswapV3V4.sol";

contract LaunchAddressSelectionV4Test is CooketV4TestBase {
    ...
    TokenDeployerV4 private deployer;
    ...
    function setUp() public override {
        super.setUp();
        deployer = TokenDeployerV4(factory.tokenDeployer());
    }
    ...
    function testCreate2AddressMatchesIndependentComputation() public view {
        ...
        bytes32 initCodeHash = keccak256(
            abi.encodePacked(
                type(CooketTokenV4).creationCode, abi.encode(address(factory), creator, "Independent", "IND")
            )
        );
    ...
    function testArcDomainSeparatesLaunchSeedsFromLegacyBasePredictions() public view {
        ...
        bytes32 arcDomain = keccak256("COOKET_ARC_V2");
    ...
    function testSdkSolidityArcCreate2VectorParity() public pure {
        ...
        assertEq(launchSeed, 0x959759fe35b4235ae42367f403aae04b43cb970c04c806999f2a6dae98a7a67b);
        assertEq(candidateSalt, 0xe78a8e01b95716d7111fe7029fa6038f487ffebc18e7ed80d84213bcc977f2f8);
        assertEq(initCodeHash, 0x848b9c6ad3911a15a45bf3091f5b234708a3d45faae4b3972836ba9274722d9b);
        assertEq(predicted, 0xA44a4588bBBaB10194323e7D13067019778d46Da);
    }

    function testLaunchPriceConstantsAreIndependentlyDerivedForBothOrderings() public view {
        uint256 terminalUsdc6PriceNumerator = 72_450;
        ...
        assertEq(derivedToken0Launch, 674_370_833_242_930_978_215);
        assertEq(derivedToken0Usdc, 9_308_086_035_099_116_331_471_807_827_060_668_654);
        ...
        assertEq(EndpointConstantsV4.TERMINAL_NATIVE_USDC_PRICE * 1_000_000_000, 72_450 ether);
        assertEq(EndpointConstantsV4.INITIAL_NATIVE_USDC_PRICE * 1_000_000_000, 4_528.125 ether);
    ...
    function testActualPoolReservationSupportsBothOrderingBranches() public {
        ...
        (CooketTokenV4 lowToken,) = _launchWithSalt(creator, "Token First", "TF", lowSalt);
        (CooketTokenV4 highToken,) = _launchWithSalt(creator, "USDC First", "UF", highSalt);
    ...
    function testNoPoolCandidateIsSelectedAndReserved() public {
        ...
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "No Pool", "NOP", keccak256("no-pool"));
    ...
    function testExistingUninitializedPoolIsAcceptedAndInitializedExactly() public {
        ...
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Uninitialized", "UNI", salt);
        assertEq(address(launchedToken), predicted);
    ...
    function testExistingInitializedExactPriceZeroActiveLiquidityPoolIsRejected() public {
        ...
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Exact Existing", "EXT", salt);
    ...
    function testExistingInitializedExactPriceActiveLiquidityPoolIsRejected() public {
        ...
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Exact Active", "EXA", salt);
        assertEq(address(launchedToken), second);
    }
    ...
    function testWrongPricePoolIsSkippedAndPublicSaltFrontRunRecovers() public {
        ...
        MockUniswapV3PoolV4(wrongPool).initialize(1);
        (address second,,) = _candidate("Front Run", "FRT", salt, 1);

        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Front Run", "FRT", salt);
        assertEq(address(launchedToken), second);
        assertNotEq(address(launchedToken), first);
    }
    ...
    function testNonzeroLiquidityPoolIsSkipped() public {
        ...
        MockUniswapV3PoolV4(occupied).setLiquidity(1);
        (address second,,) = _candidate("Liquidity", "LIQ", salt, 1);
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Liquidity", "LIQ", salt);
        assertEq(address(launchedToken), second);
    }
    ...
    function testRawUsdcDonationDoesNotRejectPristinePool() public {
        ...
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Donation", "DON", salt);
        assertEq(address(launchedToken), predicted);
    }
    ...
    function testFirstUnsafeSecondAccepted() public {
        ...
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Skip One", "SKP", salt);
        assertEq(address(launchedToken), second);
    }
    ...
    function testIndex254UnsafeAndIndex255Accepted() public {
        ...
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Last Candidate", "LST", salt);
    ...
    function testAll256UnsafeRollsBackAndFreshSaltSucceeds() public {
        ...
        vm.prank(creator);
        vm.expectRevert(abi.encodeWithSelector(ITokenDeployerV4.NoAcceptableTokenAddress.selector, launchSeed));
    ...
    function testMalformedAndNoncanonicalPoolRelationshipsAreRejected() public {
        ...
        for (uint16 i; i < 5; ++i) {
            ...
            if (i == 0) {
                ...
                configuredPool = address(
                    new MockConfiguredUniswapV3PoolV4(
                        address(0xBAD),
                        token0,
                        token1,
                        10_000,
        ...
        (CooketTokenV4 launchedToken,) = _launchWithSalt(creator, "Relationships", "REL", salt);
    ...
    function testRegistrationTimeRecheckRejectsChangedPoolStateAndRollsBack() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IGraduationManagerV4.UnsafePoolCandidate.selector,
                IGraduationManagerV4.PoolCandidateState.IncorrectPrice,
                pool
            )
        );
        factory.createToken("Recheck", "RCK", salt);

        (uint160 sqrtPriceX96,,,,,,) = MockUniswapV3PoolV4(pool).slot0();
    ...
    function testUnauthorizedDeployerCallsAndImmutableIdentities() public {
        vm.prank(buyer);
        vm.expectRevert(ITokenDeployerV4.UnauthorizedFactory.selector);
        deployer.deployToken(buyer, keccak256("unauthorized"), "Unauthorized", "UNA");

        CurveDeployerV4 curveChildDeployer = CurveDeployerV4(factory.curveDeployer());
        vm.prank(buyer);
        vm.expectRevert(ICurveDeployerV4.UnauthorizedFactory.selector);
    ...
    function testZeroSaltAndDuplicateSuccessfulDefinitionReject() public {
        vm.prank(creator);
        vm.expectRevert(ICooketFactoryV4.InvalidUserSalt.selector);
    ...
    function testStableLaunchEventContainsPoolAndSelectionData() public {
        ...
        vm.recordLogs();
        (CooketTokenV4 launchedToken, CooketCurveV4 launchedCurve) =
            _launchWithSalt(creator, "Event Token", "EVT", userSalt);
        Vm.Log[] memory logs = vm.getRecordedLogs();
        bytes32 signature = keccak256(
            "TokenLaunchedV4(address,address,address,string,uint256,uint256,uint256,address,address,bytes32,bytes32,uint16)"
        );
        bool found;
        for (uint256 i; i < logs.length; ++i) {
            ...
            assertEq(eventData.version, "endpoint-cp-v4");
    ...
    function testRuntimeAndInitcodeSizeGates() public view {
        ...
        assertLt(type(CooketFactoryV4).creationCode.length + 64, EIP_3860_MAX);
    }
    ...
    function testRealisticAll256RejectedGasBelowConservativeCap() public {
        ...
        (bool ok, bytes memory returnData) =
            address(factory).call(abi.encodeCall(ICooketFactoryV4.createToken, ("Gas Revert", "GRV", salt)));
        uint256 gasUsed = gasBefore - gasleft();
        emit log_named_uint("realistic all 256 rejected gas", gasUsed);
        assertFalse(ok);
        assertEq(bytes4(returnData), ITokenDeployerV4.NoAcceptableTokenAddress.selector);
        ...
        returns (CooketTokenV4 launchedToken, CooketCurveV4 launchedCurve)
    {
        ...
        return (CooketTokenV4(tokenAddress), CooketCurveV4(payable(curveAddress)));
    }
    ...
    function _assertExactPool(address launchToken, address pool) private view {
        MockUniswapV3PoolV4 typedPool = MockUniswapV3PoolV4(pool);
    ...
    function _createInitializedCanonicalPool(address candidate, bool activeLiquidity) private returns (address pool) {
        ...
        MockUniswapV3PoolV4(pool).initialize(graduationManager.expectedSqrtPriceX96(candidate));
        if (activeLiquidity) MockUniswapV3PoolV4(pool).setLiquidity(1);
    }
