...
import {IUniswapV3FactoryMinimal} from "../v3/interfaces/uniswap/IUniswapV3FactoryMinimal.sol";
...
/// Arc endpoint-cp-v4 graduation manager. It reserves and initializes the
...
abstract contract GraduationManagerV4Boundary is IGraduationManagerV4 {
    ...
    /// @dev floor(sqrt((72_450 / 1e21) * 2^192)); launch token (18 decimals) is token0.
    uint160 public constant SQRT_PRICE_X96_TOKEN0_LAUNCH = 674_370_833_242_930_978_215;
    /// @dev floor(sqrt((1e21 / 72_450) * 2^192)); canonical USDC (6 decimals) is token0.
    uint160 public constant SQRT_PRICE_X96_TOKEN0_USDC = 9_308_086_035_099_116_331_471_807_827_060_668_654;
    ...
    constructor(address uniswapV3Factory_, address canonicalUsdc_, address governance_) {
        if (
            uniswapV3Factory_ == address(0) || uniswapV3Factory_.code.length == 0 || canonicalUsdc_ == address(0)
                || canonicalUsdc_.code.length == 0 || governance_ == address(0)
        ) {
    ...
    function setFactoryOnce(address factory_) external override {
        ...
        if (ICooketFactoryV4(factory_).protocolVersionHash() != PROTOCOL_VERSION_HASH) {
