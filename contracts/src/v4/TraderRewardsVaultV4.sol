...
import {ITraderRewardsDistributorV4} from "./interfaces/ITraderRewardsDistributorV4.sol";
...
contract TraderRewardsVaultV4 is ITraderRewardsVaultV4, ReentrancyGuard {
    using SafeERC20 for IERC20;

    bytes32 private constant PROTOCOL_VERSION_HASH = keccak256("endpoint-cp-v4");
    ...
    constructor(address governance, address feeManager_) {
        if (
            governance == address(0) || feeManager_ == address(0) || feeManager_.code.length == 0
                || IFeeManagerV4(feeManager_).protocolVersionHash() != PROTOCOL_VERSION_HASH
                || IFeeManagerV4(feeManager_).feePolicyHash() != EndpointConstantsV4.FEE_POLICY_HASH
    ...
    function setDistributorOnce(address distributor_) external override {
        ...
        if (
            distributor_ == address(0) || distributor_.code.length == 0
                || ITraderRewardsDistributorV4(distributor_).rewardsVault() != address(this)
                || ITraderRewardsDistributorV4(distributor_).protocolVersionHash() != PROTOCOL_VERSION_HASH
                || ITraderRewardsDistributorV4(distributor_).feePolicyHash() != EndpointConstantsV4.FEE_POLICY_HASH
        ) revert InvalidDependency();
    ...
    function setPermanentLPFeeVaultOnce(address vault) external override {
        ...
        if (
            vault == address(0) || vault.code.length == 0
                || IPermanentLPFeeVaultV4(vault).traderRewardsVault() != address(this)
                || IPermanentLPFeeVaultV4(vault).feeManager() != feeManager
                || IPermanentLPFeeVaultV4(vault).feePolicyHash() != EndpointConstantsV4.FEE_POLICY_HASH
        ) revert InvalidDependency();
