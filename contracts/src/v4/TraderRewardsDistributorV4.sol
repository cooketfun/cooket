...
/// additional funding.
contract TraderRewardsDistributorV4 is ITraderRewardsDistributorV4, Ownable2Step, ReentrancyGuard {
    ...
    constructor(address governance, address rewardsVault_) Ownable(governance) {
        if (
            rewardsVault_ == address(0) || rewardsVault_.code.length == 0
                || ITraderRewardsVaultV4(rewardsVault_).protocolVersionHash() != PROTOCOL_VERSION_HASH
                || ITraderRewardsVaultV4(rewardsVault_).feePolicyHash() != EndpointConstantsV4.FEE_POLICY_HASH
        ...
        nonReentrant
    {
        ...
        ITraderRewardsVaultV4(rewardsVault).payout(launchToken, asset, msg.sender, amount);
