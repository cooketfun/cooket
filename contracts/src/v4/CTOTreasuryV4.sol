...
contract CTOTreasuryV4 is ICTOTreasuryV4, ReentrancyGuard {
    ...
    function confirmCTO(bytes32 proposalId) external override onlyController {
        ICTORegistryV4(registry).confirmCTO(proposalId);
        emit CTOConfirmationSubmitted(proposalId, controller, address(this));
    }
    ...
    function pullCurveCreatorFees() external override nonReentrant returns (uint256 amount) {
        _requireActive();
        IFeeManagerV4 fees = IFeeManagerV4(ICTORegistryV4(registry).feeManager());
    ...
    function pullLPCreatorFees(address asset) external override nonReentrant returns (uint256 amount) {
        ...
        address manager = address(ICooketFactoryV4(factory).graduationManager());
        address vault = IGraduationManagerV4(manager).permanentLPFeeVault();
        (uint256 protocolAmount, uint256 creatorAmount) = IPermanentLPFeeVaultV4(vault).claimLPFees(asset);
