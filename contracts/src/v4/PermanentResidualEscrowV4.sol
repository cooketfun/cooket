...
contract PermanentResidualEscrowV4 is IPermanentResidualEscrowV4 {
    ...
    constructor(address launchToken_, address graduationManager_, address settlementExecutor_, address canonicalUsdc_) {
        if (
            launchToken_ == address(0) || graduationManager_ == address(0) || settlementExecutor_ == address(0)
                ...
                || settlementExecutor_.code.length == 0 || canonicalUsdc_ == address(0)
                || canonicalUsdc_.code.length == 0
    ...
    function depositNativeUsdcDust() external payable override {
        ...
        if (msg.value == 0 || msg.value >= ArcNativeUsdcV4.NATIVE_PER_ERC20_BASE_UNIT) {
