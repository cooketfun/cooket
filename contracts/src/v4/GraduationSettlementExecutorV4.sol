...
contract GraduationSettlementExecutorV4 is IGraduationSettlementExecutorV4 {
    ...
    constructor(address manager_, address positionManager_) {
        address canonicalUsdc_ = IGraduationManagerV4(manager_).canonicalUsdc();
        if (
            manager_ == address(0) || positionManager_ == address(0) || manager_.code.length == 0
                || positionManager_.code.length == 0 || canonicalUsdc_ == address(0) || canonicalUsdc_.code.length == 0
    ...
    function execute(
        ...
        uint256 usdcAmount6 = ArcNativeUsdcV4.nativeUsdcToUsdc6Exact(nativeUsdcAmount);
        ...
        uint256 nativeUsed18 = ArcNativeUsdcV4.usdc6ToNativeUsdc(usedUsdc6);
        if (
        ...
        result.nativeUsdcDust18 = nativeUsdcAmount - ArcNativeUsdcV4.usdc6ToNativeUsdc(usdcAmount6);
        if (result.nativeUsdcDust18 != 0) {
            IPermanentResidualEscrowV4(residualEscrow).depositNativeUsdcDust{value: result.nativeUsdcDust18}();
        }
        if (
